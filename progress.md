...
- Follow-up optimized fort-city and settlement-village buildings with chunk/cell cached static structure placement plus `ANGLE_instanced_arrays` rendering: variant geometry is uploaded once per WebGL context, visible buildings upload only per-instance transform/color data, and unsupported browsers keep the old merged-mesh fallback.
- Follow-up restored `land.png` as the city ground texture for the built-in Yuanmo map and Zhu Yuanzhang scenario pack, added grass feathering at city-ground hex edges, kept terrain/model backlighting at the previous values, and added projected shadows for fort-city buildings and the campaign actor model. Building shadows now use compact soft blob projection from visible model footprint so they do not cover the city ground texture.
