...
### Campaign Map Node Meshes

Campaign map node meshes use `format: "campaign-map-node-mesh-v1"`. They are pure visual assets for map nodes and must not replace `maps.json.nodes`, `cities.json`, travel grid data, or city-entry rules.

The format contains baked arrays (`positions`, `normals`, `uvs`, `indices`), material `drawGroups`, relative texture URLs, and a small `placement` block for world scale, rotation, planar offset, and lift. Runtime renderers load this converted JSON plus its texture images; they must not parse OBJ/MTL directly.

The current Yuanmo fort wall asset is generated by `tools/convert-campaign-fort-wall-obj.mjs`: it consumes the already complete `part_02_left_middle_wall_large_hex_bent_subdiv_centered_tight_corner_large_battlements_uv_fixed.obj/.mtl` Hex wall model, triangulates the OBJ faces, preserves material UVs, generates normals when the OBJ has none, uniformly scales the full model to the campaign Hex footprint, applies `placement.rotationDegrees`, applies the small `placement.offsetY` centering correction, and writes one reusable `fort-hex-wall.json` mesh. The converter separates the fit target (`FORT_WALL_FIT_INNER_HEX_RADIUS`) from the true global model multiplier (`FORT_WALL_UNIFORM_SCALE`); use the latter for whole-model size comparisons, because the fit target is only the Hex footprint reference. The renderer anchors this complete visual model to the center of each loaded `kind: "fort"` Hex that is also `terrain: "平原"` while the runtime marker layer owns click and label DOM. OBJ material UVs may exceed `0..1`; runtime fort-wall textures therefore use repeatable texture upload instead of clamped sampling.
