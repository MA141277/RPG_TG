...
### Added
- ???????????`src/domain/trade-good.ts`?`src/domain/market.ts`
- ??????????????`src/content/markets/global-goods-pool.ts`?`src/application/markets/*`
- ????????????????????????????????? `GameState.runtime.cityMarkets`

### Changed
- `CityDefinition` ??? `tags / prosperity / danger / specialDemand`???????????????????
- `create-initial-state` ?????? `cityMarkets` ?????????????????????
- ?? special house ??????????????/????????????????????????
- ?? House ???????? NPC ????`activityLocationId` ???? special house ????? market / street ? house ???????? NPC

### Impact
- `mapchange` ?????house registry?tea-house / tavern ???????????????????????
- ??????? special house ???????? `cityMarkets` ? `MarketShopType`???????????????
