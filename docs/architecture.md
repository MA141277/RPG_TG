...
Campaign 地图的陆地表面材质可以通过 `map_grass_texture`、`map_sand_texture`、`map_rock_texture` 等纯表现图层传给 terrain shader。此类图层只能影响地表颜色、纹理和海岸/地貌过渡表现；陆地/水体语义、寻路、点击、探索和云洞仍必须以 `campaignHexGridUrl` 指向的 Hex 数据图与 runtime 状态为准。`map_rock_texture` 必须由 Hex 数据图中的 `terrain: "山脉"` 驱动，shader 只能在当前 rounded Hex cell 已经是山脉时显示岩石材质，并在山脉格内部沿非山脉邻边向内羽化；不得把山脉 mask 平均、膨胀或扩散到非山脉格，也不得反向从岩石贴图或 shader 颜色推断 gameplay terrain。真实山脉起伏由 CPU 派生高度场生成：`map_heights` 的原始采样值必须直接作为山体基准高度，不得做山脉专用对比度拉伸；最终山体必须按 terrain 网格采样点生成连续山脉内部的多控制点主脊、支脊和沟壑，噪声只能在原始高度上追加有限细节，不能退化成每个 Hex 一个随机高度、一个孤立峰值、按格断开的山包或整格高原块，也不能用固定 relief 抹平 `map_heights` 的高低差；山脚与岩石材质过渡仍由既有 `getLocalMountainEdgeInset` 负责，山体形状规则不得另起一套山脚过渡或材质 body mask。当前标准先不做雪线/积雪，后续若加入积雪必须作为独立表现贴图或高处 mask 层追加。相邻山脉格必须作为整体连续处理，内部山脉邻接边不压低，只有整片山脉外边界按边界距离和噪声向内压低；非山脉格不执行山脉专用高度规则，只通过通用地形连续性与相邻高度自然衔接。Campaign terrain renderer 会在地形光照之后应用统一的历史色调层：保留材质原本色相，轻微暖化并压住过亮高光，避免单个材质或植被资产以高亮度破坏大地图厚重感，也不得把整体压成失去地貌颜色的灰图。
