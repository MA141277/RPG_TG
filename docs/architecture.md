...
Campaign 地图的迷雾探索状态来自 `runtime.mapExplorationByMapId`。未探索 hex 的点击屏蔽、marker 交互屏蔽必须在应用交互层完成，和水格通行检查保持独立；视觉云雾 renderer 只能消费探索状态生成遮罩，不得修改探索、寻路、地形高度、水体材质或通行网格。`campaign-terrain-webgl.ts` 负责地形投影、相机、通行网格和投影点同步；`campaign-cloud-webgl.ts` 负责视口级云雾 overlay，并通过 `campaign-terrain-webgl.ts` 的只读固定云洞高度投影 helper 对齐累计已探索 hex。云洞 reveal mask 必须跟随当前 terrain camera 的平移、缩放和视口尺寸，但不得采样 `map_heights` 或使用逐格高度锚点投影；玩家、marker、hover 描边等贴地对象仍可使用高度锚点投影。探索揭示与地形加载节奏必须分离：云层可以立刻读取新增 revealed hex 播放消散，terrain renderer 只能继续显示已加载 chunk，延迟期间完成的 chunk 也要等动画窗口结束后再上传；云层实际启动 dissolve transition 时必须持有 terrain 加载暂停令牌，并在 transition 完成后延迟释放，不能只按玩家移动结束时间或固定毫秒恢复。地图重绘时必须保留 terrain、actor、cloud canvas 和 marker layer；campaign marker 的全量数据可以保留为 JSON 数据源，但 marker / summary DOM 必须由 terrain renderer 根据当前已加载 chunk 增量维护，避免全图城市入口 DOM 在首屏初始化时生成，也避免移动刷新时闪烁。可踏足 hex 的悬浮描边属于地图视口内的临时表现反馈，只能读取地形投影、探索状态和 navigation 寻路结果，不得写入 gameplay 状态。层级契约是：地图底图、建筑点本体、玩家 DOM sprite 和 actor canvas 在云层下方；可踏足 hex 悬浮描边、marker 悬浮详情、地图 debug 控件、全局 UI 和确认 modal 在云层上方。视觉算法细节属于 renderer 源码和 changelog，不应写入架构文档。
