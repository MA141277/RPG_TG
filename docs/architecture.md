...
Campaign 地图的陆地表面材质可以通过 `map_grass_texture`、`map_sand_texture`、`map_rock_texture`、`map_snow_texture` 等纯表现图层传给 terrain shader。此类图层只能影响地表颜色、纹理和海岸/地貌过渡表现；陆地/水体语义、寻路、点击、探索和云洞仍必须以 `campaignHexGridUrl` 指向的 Hex 数据图与 runtime 状态为准。整体地表明暗必须来自当前最终 chunk 高度派生的高度、法线和 terrain 光照参数，不得再从原始地表 atlas 的亮度采样中继承旧地图阴影或高光。`map_rock_texture` 必须由 Hex 数据图中的 `terrain: "山脉"` 驱动，但山脉格不能无条件渲染成 100% 岩石底色；shader 应按当前山脉格的高度、坡度和噪声形成岩石主体，并只在相邻非山脉陆地格靠共享山脉边缘生成受限岩石过渡带；不得把山脉 mask 平均、膨胀或扩散到第三格，也不得反向从岩石贴图或 shader 颜色推断 gameplay terrain。`map_snow_texture` 是叠加在山脉岩石上的高处雪顶表现层，必须同时受当前山脉格 mask、最终 chunk 高度派生的 fragment 高度、坡面朝上程度和细节噪声控制；雪线只能在山脉格内部形成类似文明的山顶积雪与破碎雪缘，不得扩散到非山脉格、低山山脚或水陆/海岸语义层。真实山脉起伏由 CPU 派生高度生成：山脉格的基准层必须来自普通陆地高度连续性，也就是从非山地陆地高度扩散并平滑得到山地底面；Hex `referenceHeight` 只能作为山体增量强度和高度上限的参考输入，不得直接作为整片山地基准层把山脉格整体抬高，也不得做山脉专用对比度拉伸。最终山体必须按 terrain 网格采样点生成连续山脉内部的世界空间峰体、主脊、次脊和沟壑，山峰/山脊中心不得绑定到单个 Hex 本地坐标；细节噪声可以使用基于局部梯度幅值衰减 octave 权重的 fBm 辅助侵蚀感，但不能取代 CPU 几何主形状。山体不能退化成每个 Hex 一个随机高度、一个孤立峰值、按格断开的山包、针状尖峰、整格高原块或只是皱褶化的鼓包；山脚岩石材质过渡由非山脉邻格的共享边过渡带负责，山体形状规则不得另起一套材质 body mask。相邻山脉格必须作为整体连续处理，内部山脉邻接边不压低，山脉高度规则不得因为外边界在山脉格内部强行压低峰体；非山脉陆地格在进入山脉高度生成前必须按单格平均高度做内部平整化，并只在同一 Hex 内做小范围平滑，削弱 Hex `referenceHeight` 在平原、草地、森林等格之间留下的尖锐起伏；非山脉格不执行山脉专用高度规则，只通过通用地形连续性与相邻高度自然衔接。Campaign terrain renderer 会在地形光照之后应用统一的历史色调层：保留材质原本色相，轻微暖化并压住过亮高光，避免单个材质或植被资产以高亮度破坏大地图厚重感，也不得把整体压成失去地貌颜色的灰图。
