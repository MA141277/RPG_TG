...
- `The current target is now back to idle-open with no active queue. The remaining runtime, package, composition, and cleanup items stay at target-level candidate review only until a later same-target admission review selects the next lawful queue.`
