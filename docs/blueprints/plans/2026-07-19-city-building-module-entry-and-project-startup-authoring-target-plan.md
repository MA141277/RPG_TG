...
- active_queue: `queue.script-editor-city-building-location-access-condition-authoring-correction`
- decision_state: `active-execution`
- next_decision: `queue-closeout-or-return-to-version-review`
- next_action: `write-queue-closeout`
- resume_gate: `execution`
...
- candidate_backlog_snapshot:
  ...
  - `queue.script-editor-main-ui-flow-mojibake-repair-and-encoding-guard-hardening: recorded-only candidate for main-ui-flow.js mojibake repair and encoding guard hardening after the active queue closes.`
...
- Activation conclusion:
  ...
  - `queue.script-editor-city-building-location-access-condition-authoring-correction is the active queue.`
  - `task.script-editor-city-building-location-access-condition-authoring-correction.evidence-anchor-reconcile is the current task.`
...
| `item.script-editor-city-building-location-access-condition-authoring-correction` | `queue-candidate` | `queue.script-editor-city-building-location-access-condition-authoring-correction` | `admitted` | `active queue admitted and evidence-anchor-reconcile is live` | `ACC-CITY-BUILDING-ACCESS-CONDITION-001..007` | `src/ui/main-ui/main-ui-flow.js; src/ui/views/script-editor/**; src/application/script-editor/**; src/domain/script-editor-project.ts; src/application/location-access/**; src/application/script-editor/runtime-pack-export.ts; src/application/scenario/scenario-pack-loader.ts; src/core/runtime/navigation-runtime.ts; tests/**; browser simulated-human flow` | `city/building entry-condition authoring correction; refusal prompt select; event/person/time condition factors; runtime-understandable locationAccess export/load/evaluation; city and building simulated-human coverage` | `EventBindingRuntime semantics; unrelated event-binding queues; map/review provider cleanup; broad list/search UX; reopening closed enter-state queue` | `Promoted from MEMO-014 on explicit operator request and now admitted into active execution under the current open version.` |
| `item.script-editor-city-building-location-access-refusal-runtime-handoff-correction` | `queue-candidate` | `queue.script-editor-city-building-location-access-refusal-runtime-handoff-correction` | `recorded-only` | `after queue.script-editor-city-building-location-access-condition-authoring-correction closes, verify whether refusal handoff was absorbed; admit only if city/building refusal overlay remains unresolved` | `ACC-CITY-BUILDING-ACCESS-REFUSAL-RUNTIME-001..005` | `src/core/runtime/navigation-runtime.ts; src/core/contracts/runtime-result.ts; src/main.ts; src/application/runtime/city-house-transition-coordinator.ts; src/application/city/city-building-placement-resolver.ts; src/application/city/city-building-house-runtime-adapter.ts; tests/**; browser simulated-human flow` | `routeNavigationRuntime must preserve access.refusal; main shell must convert city refusal into locationDialogueState; building entry must consume locationAccessDefinitions without falling back only to legacy houseAccessRefusalRules; simulated-human city/building refusal prompt proof` | `Script Editor condition picker redesign; broad encoding cleanup; EventBindingRuntime changes; map/review provider cleanup` | `Recorded from operator-approved draft after evidence showed city refusal is lost at routeNavigationRuntime result transfer while building has a separate main-entry path risk.` |
| `item.script-editor-main-ui-flow-mojibake-repair-and-encoding-guard-hardening` | `queue-candidate` | `queue.script-editor-main-ui-flow-mojibake-repair-and-encoding-guard-hardening` | `recorded-only` | `after the active queue closes, admit if main-ui-flow.js still contains mojibake or encoding guard coverage remains insufficient` | `ACC-MAIN-UI-MOJIBAKE-001..006` | `src/ui/main-ui/main-ui-flow.js; src/ui/views/script-editor/**; src/application/script-editor/**; tools/check-ui-encoding-integrity.mjs; package.json; tests/robustness.test.cjs; browser simulated-human flow` | `repair all identifiable hardcoded mojibake in main-ui-flow.js; repair damaged HTML strings such as broken option tags; clarify UTF-8-vs-mojibake distinction in guards; expand lint:encoding coverage; browser-proof critical Script Editor pages render Chinese correctly` | `unrelated UI redesign; broad main-ui-flow.js architectural split unless required to repair text; gameplay/runtime behavior changes; EventBindingRuntime changes` | `Recorded from operator-approved draft after evidence found about 295 suspicious mojibake lines and confirmed the file can still be UTF-8 while containing already-corrupted hardcoded strings.` |
...
| `queue.script-editor-city-building-location-access-condition-authoring-correction` | `active` | `evidence-anchor reconcile is complete and implementation may begin` | `Admitted from MEMO-014 into active execution under the current open version. The queue doc now exposes live active task truth.` |
| `queue.script-editor-city-building-location-access-refusal-runtime-handoff-correction` | `recorded-only` | `after the active queue closes and version-level admission review confirms it was not absorbed` | `Candidate recorded while queue.script-editor-city-building-location-access-condition-authoring-correction remains active; do not activate until current queue closeout/handoff returns to version review.` |
| `queue.script-editor-main-ui-flow-mojibake-repair-and-encoding-guard-hardening` | `recorded-only` | `after the active queue closes and version-level admission review confirms encoding repair remains pending` | `Candidate recorded while queue.script-editor-city-building-location-access-condition-authoring-correction remains active; do not activate until current queue closeout/handoff returns to version review.` |
