...
- closure_review_subject: `queue.event-binding-runtime-convergence`
- closure_review_status: `routed`
- residue_candidate_id: `item.old-event-runtime-retirement`
- residue_candidate_family: `same-family`
- routing_basis: `EventBindingRuntime selector and TriggerContext story adapter cutover landed and verified for built-in/exported double-table inputs; old events[].trigger/conditions scanning paths remain only as compatibility residue and can now be routed to retirement.`
- next_lawful_queue_recommendation: `queue.old-event-runtime-retirement`
