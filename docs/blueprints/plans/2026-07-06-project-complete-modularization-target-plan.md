...
- active_queue: `queue.cross-mechanism-composition-contract-closure`
...
- candidate_queue_ids:
  ...
  - `queue.layout-editor-retirement-and-reference-removal`
...
| `queue.layout-editor-retirement-and-reference-removal` | `candidate` | `only if a fresh audit still proves the layout editor remains a live feature family whose complete removal needs one bounded queue instead of dead cleanup only` | `Fresh 2026-07-09 source audit shows the layout editor still exists as a live user-facing feature and code path across src/application/layout-editor/**, src/ui/tools/layout-editor-view.ts, src/ui/app-render.ts, src/ui/main-ui/main-ui-flow.js, src/content/layout-editor-presets.ts, src/styles/layout-editor.css, and app-shell state. Because the current version already has another active queue, this remains candidate-only until later promotion review rather than becoming a second active queue.` |
...
| `queue.cross-mechanism-composition-contract-closure` | `active` | `only if a fresh audit still proves menu/dialog/map/city/house/story/task/minigame composition remains spread across owner-specific coordinators and direct state writes and version-level review later selects a dedicated composition queue` | `Fresh 2026-07-09 source audit still shows the covered mechanism composition surface lacks one contract-driven composition seam. src/application/runtime/main-runtime-orchestrator.ts, src/application/runtime/interactive-action-coordinator.ts, src/application/runtime/city-house-transition-coordinator.ts, src/application/house/house-runtime.ts, src/application/story-battle/story-battle-runtime.ts, and src/main.ts still each own cross-mechanism composition outcomes such as view switching, dialogue opening, house reentry, story advancement, battle completion routing, and render-triggered orchestration, so this queue is now admitted as the single active queue.` |
...
| `item.layout-editor-retirement-and-reference-removal` | `queue-candidate` | `queue.layout-editor-retirement-and-reference-removal` | `candidate-only` | `Fresh 2026-07-09 source audit shows the layout editor remains a live feature rather than historical residue only: app shell state still carries layoutEditor plus uiLayouts, startup bootstrap still seeds layout editor defaults, UI rendering still mounts renderLayoutEditor, live layout bindings and character detail rendering still expose editor-aware branches, and dedicated layout-editor modules, presets, docs, tests, and CSS remain in the covered repository path. The request is therefore queue-shaped, but current Blueprint truth already has queue.cross-mechanism-composition-contract-closure active, so this new queue must stay candidate-only until later promotion review.` |
...
| `item.cross-mechanism-composition-contract-closure` | `queue-candidate` | `queue.cross-mechanism-composition-contract-closure` | `admitted + active` | `Fresh 2026-07-09 source audit still shows menu or dialogue or map or city or house or story or task or minigame composition lacks one contract-driven combination seam. src/application/runtime/main-runtime-orchestrator.ts still combines startup-session, story-scene advance, choice handling, and trigger-story-events ownership; src/application/runtime/interactive-action-coordinator.ts still combines activity-qte, story-scene, and story-battle action flow; src/application/runtime/city-house-transition-coordinator.ts and src/application/house/house-runtime.ts still own cross-mechanism view and dialogue transitions; src/application/story-battle/story-battle-runtime.ts still returns story, house, and review outcomes directly; and src/main.ts still assembles and bridges all of these owners. The queue is now admitted so baseline-reconcile can freeze the smallest lawful first composition seam before implementation widens.` |
...
- Admission basis:
  - `queue.ui-runtime-contract-consumption was admitted only after the version plan and queue doc were synchronized with written runtime-facing UI contract bypass evidence.`
...
- Current review subject:
  - `none; queue.cross-mechanism-composition-contract-closure is now the single active queue and version control has moved into active execution.`
- Current handoff:
  - `Fresh 2026-07-09 promotion review now admits queue.cross-mechanism-composition-contract-closure as the next active queue because current source truth still shows view/dialogue/house/story/battle composition spread across multiple owners with no shared composition seam.`
  - `Current source truth still shows src/application/runtime/main-runtime-orchestrator.ts combining startup-session application, story-scene advance, choice handling, and trigger-story-events ownership; src/application/runtime/interactive-action-coordinator.ts combining activity-qte, story-scene, story-battle, and reentry follow-up flow; src/application/runtime/city-house-transition-coordinator.ts and src/application/house/house-runtime.ts writing direct view/dialogue/house transitions; src/application/story-battle/story-battle-runtime.ts writing review plus house-or-scene return routing; and src/main.ts still assembling these owners together.`
  - `The first legal execution point is task.cross-mechanism-composition-contract-closure.baseline-reconcile so the queue can freeze the smallest lawful first composition seam before any implementation cut begins.`
  - `Fresh 2026-07-09 source audit also records item.layout-editor-retirement-and-reference-removal as a new queue-candidate rather than dead cleanup: the layout editor still exists as a live feature surface across application state, startup bootstrap, app render, main UI flow, editor modules, presets, styles, docs, and targeted tests. Because queue.cross-mechanism-composition-contract-closure is already the single active queue, this new editor-removal line is recorded for later promotion review only and must not become a second active queue now.`
...
1. `Read project-progress -> blueprint -> version plan -> active queue before evaluating a fresh queue item.`
...
- `Use this version plan's existing queue promotion ledger and prior review fields as the default recovery source for previously recorded queue-candidates.`
...
- `2026-07-07: queue.ui-runtime-contract-consumption was closed after the bounded shared-dialog component landed, approved replacement points were consumed, and verification passed; version state returned to open with no active queue.`
...
- `2026-07-07: queue.playable-family-gap-audit was closed after playable contribution truth and activation-configurable default runtime registries landed, and verification passed; version state returned to open with no active queue.`
...
- `2026-07-08: queue.main-shell-and-layout-editor-ownerization ownerization work reached the accepted pure-shell line on fresh source evidence, and the queue was closed as historical evidence only; the known repository-wide import.meta and ?url asset typing/configuration gap remains outside that finished queue slice, so version state returned to idle-open with no active queue.`
