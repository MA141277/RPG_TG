...
- blocked_by: []
- candidate_queue_ids:
  - `queue.entry-shell-bootstrap-ownerization`
...
| `queue.entry-shell-bootstrap-ownerization` | `candidate` | `Promote first if fresh evidence still proves the entry shell lacks one lawful owner seam for main.ts/bootstrap/runtime assembly residue.` | `This is the lawful first cut for the current residue family. Fresh basis is that src/main.ts still directly owns base content activation, builtin scenario-pack wiring, prototype startup app-state bootstrap, repeated runtime commit assembly, and house-runtime creation, so the next bounded governance move is entry-shell/bootstrap ownerization before downstream runtime or content-consumption cleanup.` |
...
| `item.entry-shell-bootstrap-ownerization` | `queue-candidate` | `queue.entry-shell-bootstrap-ownerization` | `candidate-only` | `Fresh source review keeps this as the lawful first cut because src/main.ts still directly owns base content activation, builtin scenario-pack wiring, prototype startup app-state bootstrap, repeated runtime commit assembly, and house-runtime creation, so one bounded entry-shell/bootstrap ownerization queue can be admitted without widening into later runtime or content cleanup.` |
...
- Current review subject:
  - `none`
- Current handoff:
  - `Fresh 2026-07-09 queue closeout returns queue.layout-editor-retirement-and-reference-removal to closed historical evidence after the bounded queue retired the remaining layout editor surfaces, removed the dead layoutEditor-state bootstrap, and deleted the editor-only module family on the covered production path.`
  - `src/main.ts, src/application/app-shell.ts, src/application/startup/prototype-startup-app-state.ts, and src/application/layout-editor/layout-editor-bootstrap.ts now converge on the uiLayouts-only createDefaultUiLayoutAppState seam, while dead editor-only modules and styles no longer remain on the covered path.`
- `Queue-local residue review concludes that the remaining uiLayouts baseline and preset family in src/application/layout-editor/layout-editor-bootstrap.ts, src/content/layout-editor-presets.ts, and src/domain/content-pack.ts no longer forms another already-frozen same-queue cut, so version control returns to idle-open with no active queue.`
- `The next legal version action is to classify fresh work or promote a newly evidenced broader queue only if current source truth later proves one.`
...
- `Only queue.entry-shell-bootstrap-ownerization is in the current live candidate set.`
