...
- active_queue: `queue.zhuyuanzhang-pack-structure-and-authoring-normalization`
...
- intake_item_id: `item.zhuyuanzhang-pack-hardcode-consolidation-and-editor-prep`
- intake_summary: `New requirement plus governance review selected the package-normalization candidate as the next active queue because current source truth still shows zhuyuanzhang pack-owned defaults leaking outside the canonical scenario-pack boundary and this queue offers a smaller bounded first slice than the remaining composition and cleanup candidates.`
...
| `queue.zhuyuanzhang-pack-structure-and-authoring-normalization` | `active` | `only if a fresh audit still proves zhuyuanzhang pack-owned truth remains hardcoded outside the canonical scenario-pack boundary and target-level review later selects one bounded package-normalization queue` | `Fresh 2026-07-09 promotion review admitted and activated this queue because the bounded package-normalization blocker still holds on current source truth and is now the smallest lawful next queue. src/content/base-game-content-pack.ts still hardcodes zhuyuanzhang as the builtin default manifest path, while src/content/scenario-packs/catalog.json and src/content/scenario-packs/scenario-pack-catalog.ts already expose builtin catalog default truth that can close the first slice without widening immediately into the broader composition candidate. Remaining pack-content-access.ts, prototype-world.ts, and src/main.ts residue stays in-queue for later bounded review after the default-pack binding cut lands.` |
...
| `item.zhuyuanzhang-pack-hardcode-consolidation-and-editor-prep` | `queue-candidate` | `queue.zhuyuanzhang-pack-structure-and-authoring-normalization` | `admitted + active` | `Fresh 2026-07-09 source review confirms the zhuyuanzhang package still lacks one canonical package-owned authoring boundary. src/content/base-game-content-pack.ts still hardcodes zhuyuanzhang as the builtin default manifest path, src/content/pack-content-access.ts still hard-imports zhuyuanzhang activities or events or scenes or text and remaining house JSON directly, src/content/prototype-world.ts still carries large zhuyuanzhang-owned scenario truth in TypeScript instead of pack-local JSON, and src/main.ts still seeds zhuyuanzhang-specific prototype and haozhou-return startup assumptions. The first admitted slice is to freeze and then close the builtin default-pack binding through existing scenario-pack catalog truth before deciding whether the remaining package-entry residue continues in-queue or returns to target review.` |
...
- Current review subject:
  - `none; queue.zhuyuanzhang-pack-structure-and-authoring-normalization is now the active queue, so admission review is no longer live in the target control block.`
- Current handoff:
  - `Fresh 2026-07-09 queue closeout returned control from queue.runtime-contract-registry-seam-closure to target-level promotion-review, and the same-day promotion review now admits queue.zhuyuanzhang-pack-structure-and-authoring-normalization as the single active queue. The current bounded package-normalization blocker still holds across src/content/base-game-content-pack.ts, src/content/pack-content-access.ts, src/content/prototype-world.ts, src/main.ts, and builtin scenario-pack catalog truth.`
  - `The first lawful execution slice is to freeze and then close the hardcoded builtin default-pack binding in src/content/base-game-content-pack.ts by consuming existing builtin catalog default truth rather than hardcoding zhuyuanzhang pack.json directly.`
  - `Remaining pack-content-access.ts, prototype-world.ts, and src/main.ts package-entry residue stays in-queue for later bounded review after the first slice lands; queue.cross-mechanism-composition-contract-closure, item.legacy-adapter-and-bridge-retirement, and item.home-keep-fallback-retirement remain candidate-only and are not reopened by this admission.`
  ...
  - `queue.zhuyuanzhang-pack-structure-and-authoring-normalization is now the only active queue. Further same-target work must resume from its queue doc until queue closeout returns control to target review.`
