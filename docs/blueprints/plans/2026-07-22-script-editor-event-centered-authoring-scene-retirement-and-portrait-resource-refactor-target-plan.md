...
- promotion_review_result: `candidate-recorded-awaiting-admission-review`
- review_subject_id: `item.event-playable-destination-and-building-action-event-truth-convergence`
- review_subject_classification: `current-target-item`
- proposed_queue_id: `queue.event-playable-destination-and-building-action-event-truth-convergence`
- review_basis: `fresh-post-closeout-gap-audit-within-parent-spec`
- admission_status: `none`
- intake_status: `candidate-recorded`
- intake_item_id: `item.event-playable-destination-and-building-action-event-truth-convergence`
- intake_summary: `Record a same-target candidate queue for event-owned minigame/playable lowering and building action event-truth convergence without admitting implementation.`
- intake_result: `queued-as-candidate`
- intake_feedback_mode: `fixed-receipt`
- closure_review_subject: `queue.script-editor-event-centered-authoring-final-acceptance-and-residue-guard`
- closure_review_status: `routed`
- residue_candidate_id: `item.event-playable-destination-and-building-action-event-truth-convergence`
- residue_candidate_family: `same-family`
- routing_basis: `fresh-post-closeout-gap-audit-within-parent-spec`
- next_lawful_queue_recommendation: `queue.event-playable-destination-and-building-action-event-truth-convergence`
...
- candidate_queue_ids:
  ...
  - `queue.event-playable-destination-and-building-action-event-truth-convergence`
...
- candidate_backlog_snapshot:
  - `queue.script-editor-event-centered-authoring-model-unification: closed after ACC-EVENT-CENTER-001 verification and queue closeout; no same-family residue remains in its bounded authoring-model scope.`
  - `queue.event-router-only-trigger-contract-freeze: closed after ACC-EVENT-CENTER-002 verification and queue closeout; no same-family residue remains in its bounded router-freeze scope.`
  - `queue.scene-family-retirement-and-content-migration: closed after ACC-EVENT-CENTER-003 / 004 / 007 verification and queue closeout; no same-family residue remains in its bounded scene-retirement scope.`
  - `queue.event-centered-runtime-pack-preview-export-sync: closed after ACC-EVENT-CENTER-005 verification and queue closeout; no same-family residue remains in its bounded runtime/preview/export/import/loader convergence scope.`
  - `queue.event-only-routing-family-retirement-and-reference-replacement: closed after creator-facing flow-shell retirement, canonical flowPlayables family replacement, retired flowDefinitions rejection, targeted verification, and successful repository sync toward origin/mod-first-dev.`
  - `queue.event-playable-destination-and-building-action-event-truth-convergence: recorded-only same-target candidate after fresh audit proved event-owned minigame/playable destinations still fail closed as non-runnable and building action event selection still diverges from canonical runtime trigger truth. Not admitted; no implementation authorized yet.`
  - `queue.portrait-resource-authoring-and-resource-mapping-convergence: closed after ACC-EVENT-CENTER-006 verification confirmed project-owned portrait families, explicit pack-owned portrait files, shared preview/runtime mapping, and prototype startup cutover with no same-family portrait residue. The earlier portrait-only sync attempt could not be safely isolated, so the remaining completed queues were later synchronized together in one combined remote-sync batch to origin/mod-first-dev.`
  - `queue.script-editor-event-centered-authoring-final-acceptance-and-residue-guard: closed after ACC-EVENT-CENTER-008 browser/source acceptance covered cross-environment trigger proof and the minimal usable portrait creator path through the template workspace seam. The queue is now also covered by the same combined completed-queue remote-sync batch to origin/mod-first-dev.`
- candidate_backlog_scan_sources:
  ...
  - `fresh source/runtime audit for event-owned minigame destination export and building action event-truth divergence`
...
- `queue.event-only-routing-family-retirement-and-reference-replacement closeout sync is now recorded as successful: commit 8c9f1f2 pushed to origin/mod-first-dev.`
- `queue.portrait-resource-authoring-and-resource-mapping-convergence is now closed after ACC-EVENT-CENTER-006 verification and closeout review found no same-family portrait residue.`
...
- `Fresh same-version candidate intake now records queue.event-playable-destination-and-building-action-event-truth-convergence after post-closeout audit found remaining parent-spec divergence on event-owned minigame/playable lowering and building action event truth.`
- `No active queue remains on this version; the next lawful action is same-version candidate/admission review rather than version closeout.`
...
- `Closeout judgement: version closeout conditions are not yet satisfied. Fresh same-version candidate intake proved one remaining implementation-bearing gap inside the current parent spec, so version closeout cannot proceed until queue.event-playable-destination-and-building-action-event-truth-convergence is admitted, rejected, or otherwise lawfully routed.`
- `Closeout confirmation: not currently lawful because candidate/admission review now precedes any version-closeout action.`
- `Future routing: this version stays open with active_queue=none and resumes from same-version candidate/admission review rather than by silently closing or reopening a closed queue.`
...
- `Historical exception note: any queue closeout recorded before the formal repository-sync gate was written into Blueprint governance may remain as a documented historical exception if its queue doc and version plan explicitly say that remote sync was not attempted before handoff. Those historical exceptions must not be misread as compliant examples for later queue closeout behavior in this version.`
...
- receipt_queue_goal:
  - `none`
- receipt_next_step:
  - `Keep the version open and route the recorded same-target candidate through later admission review without starting implementation now.`
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `item.script-editor-event-centered-authoring-model-unification` | `queue-candidate` | `queue.script-editor-event-centered-authoring-model-unification` | `closed-local` | `none` | `No recheck is currently needed because the queue is already closed.` | `ACC-EVENT-CENTER-001` | `src/domain/script-editor-project.ts; src/application/script-editor/**; src/ui/main-ui/main-ui-flow.js` | `creator-facing event-centered semantics and destination-family ownership` | `scene retirement, trigger freeze, runtime cutover, portrait mapping` | `Closed on 2026-07-22 after typecheck, test, and blueprint-lint verification confirmed the creator-facing model moved onto event destination ownership with no same-family residue inside ACC-EVENT-CENTER-001.` |
| `item.event-router-only-trigger-contract-freeze` | `queue-candidate` | `queue.event-router-only-trigger-contract-freeze` | `closed-local` | `none` | `No recheck is currently needed because the queue is already closed.` | `ACC-EVENT-CENTER-002` | `src/domain/event.ts; EventBindingRuntime entrypoints; trigger-context contracts; tests/**` | `event-only routing truth and stable trigger timing/context contracts` | `scene content migration; portrait mapping` | `Closed on 2026-07-22 after shared trigger contract/runtime-export matrix freeze, event-only activation cutover, and full verification.` |
| `item.scene-family-retirement-and-content-migration` | `queue-candidate` | `queue.scene-family-retirement-and-content-migration` | `closed-local` | `none` | `No recheck is currently needed because the queue is already closed.` | `ACC-EVENT-CENTER-003; ACC-EVENT-CENTER-004; ACC-EVENT-CENTER-007` | `src/domain/action.ts; src/domain/script-editor-project.ts; src/application/scene/**; src/application/story/**; src/application/startup/**; src/ui/views/scene/**` | `formal scene retirement and content migration` | `runtime/export/import divergence as acceptable residue` | `Closed on 2026-07-22 after no-scene contract migration, runtime/presenter/startup truth removal, and full verification.` |
| `item.event-centered-runtime-pack-preview-export-sync` | `queue-candidate` | `queue.event-centered-runtime-pack-preview-export-sync` | `closed-local` | `none` | `No recheck is currently needed because the queue is already closed.` | `ACC-EVENT-CENTER-005` | `src/application/script-editor/runtime-pack-export.ts; src/application/script-editor/runtime-pack-import.ts; src/application/scenario/scenario-pack-loader.ts; preview/runtime loaders; tests/**` | `editor/runtime/export/import no-scene convergence` | `event-only routing-family replacement, portrait resource convergence, and final acceptance ownership` | `Closed on 2026-07-22 after startup-view fail-closed convergence, built-in runtime-pack startup cleanup, mod startup metadata alignment, and full verification.` |
| `item.event-only-routing-family-retirement-and-reference-replacement` | `queue-candidate` | `queue.event-only-routing-family-retirement-and-reference-replacement` | `closed-local` | `none` | `No recheck is currently needed because the queue is implemented, closed, and its repository sync gate was satisfied before the next queue admission.` | `ACC-EVENT-ONLY-ROUTING-001; ACC-EVENT-ONLY-ROUTING-002; ACC-EVENT-ONLY-ROUTING-003; ACC-EVENT-ONLY-ROUTING-004; ACC-EVENT-ONLY-ROUTING-005; ACC-EVENT-ONLY-ROUTING-006` | `src/domain/script-editor-project.ts; src/application/script-editor/**; src/ui/main-ui/main-ui-flow.js; src/application/script-editor/runtime-pack-export.ts; src/application/script-editor/runtime-pack-import.ts; src/application/scenario/scenario-pack-loader.ts; src/application/building/**; src/application/runtime/**; tests/**` | `event-only routing-family retirement and flow-to-event reference replacement without merging queue boundaries` | `portrait resource convergence and final acceptance ownership` | `Closed on 2026-07-22 after flowPlayables family replacement, retired flowDefinitions rejection, targeted verification, and successful push of commit 8c9f1f2 to origin/mod-first-dev.` |
| `item.event-playable-destination-and-building-action-event-truth-convergence` | `queue-candidate` | `queue.event-playable-destination-and-building-action-event-truth-convergence` | `recorded-only` | `fresh same-version admission review after current candidate record` | `Fresh post-closeout audit proved the current parent spec still has two same-family gaps: event destination family "minigame" remains authorable but not runnable in export/runtime, and building action event selection remains creator-facing metadata that does not yet converge with canonical runtime trigger truth.` | `ACC-EVENT-ONLY-ROUTING-007; ACC-EVENT-ONLY-ROUTING-008` | `src/ui/main-ui/main-ui-flow.js; src/application/script-editor/runtime-pack-export.ts; src/application/script-editor/minigame-binding-authoring.ts; src/application/script-editor/city-building-authoring.ts; src/application/building/**; src/domain/event.ts; src/core/runtime/playable-runtime.ts; src/main.ts; tests/**` | `event-owned minigame/playable lowering and building action event-truth convergence as one same-target bounded queue` | `portrait resource convergence, final acceptance ownership, or any hardcoded building business rewrite` | `Recorded on 2026-07-22 as a same-target candidate only. It is intentionally not admitted yet and must not be absorbed into final acceptance or treated as already closed by the earlier routing-family queue.` |
| `item.portrait-resource-authoring-and-resource-mapping-convergence` | `queue-candidate` | `queue.portrait-resource-authoring-and-resource-mapping-convergence` | `closed-local` | `none` | `No recheck is currently needed because the queue is closed and its later combined remote-sync result is now recorded.` | `ACC-EVENT-CENTER-006` | `src/domain/script-editor-project.ts; portrait authoring helpers/UI; resource mapping/runtime loaders; tests/**` | `portrait resources, variants, mapping, thumbnail/current preview/runtime continuity` | `final acceptance ownership` | `Closed on 2026-07-22 after project-owned portrait family convergence, explicit pack-owned portrait files, shared runtime/preview mapping, and prototype startup cutover. The earlier portrait-only sync attempt could not be safely isolated, but the inseparable remaining completed queues were later committed and pushed together to origin/mod-first-dev.` |
| `item.script-editor-event-centered-authoring-final-acceptance-and-residue-guard` | `queue-candidate` | `queue.script-editor-event-centered-authoring-final-acceptance-and-residue-guard` | `closed-local` | `none` | `No candidate recheck is currently needed because the required-final queue is now closed with no same-family residue.` | `ACC-EVENT-CENTER-008` | `tests/**; browser flow; source guards; version acceptance ledger` | `final no-over-narrowing acceptance and creator-path proof` | `primary implementation ownership` | `Closed on 2026-07-22 after browser/source acceptance covered normal start, JSON runtime-pack import, Script Editor runtime preview, representative building-enter dialogue behavior, and the minimal usable portrait creator path through the template workspace seam.` |
| `item.map-rendering-city-data-separation-and-canonical-numeric-id-transition` | `future-target-candidate` | `queue.map-rendering-city-data-separation-and-canonical-numeric-id-transition` | `recorded-only` | `explicit successor-target routing review or later post-closeout candidate screening` | `MEMO-027 exceeds the current event-centered/scene-retirement/portrait parent target because it adds city-owned map truth, maps-family retirement pressure, canonical numeric id generation, zhuyuanzhang incremental id adoption, and direct-vs-indirect id consumer cleanup. It also exceeds target.map-review-provider-boundary-extraction because that target owns provider/modularization and in-game review boundaries rather than canonical id transition and script-editor/runtime/template id governance.` | `draft-only` | `docs/blueprints/version-memo.md#memo-027-map-rendering-and-city-data-separation-with-canonical-numeric-id-transition-draft; src/application/content/active-game-content.ts; src/application/map/**; src/application/script-editor/**; src/domain/script-editor-project.ts; src/content/scenario-packs/zhuyuanzhang/**; tests/**` | `future-target source item for city-owned map truth, map render/runtime routing boundary cleanup, and canonical numeric id transition governance with consumer-route audit` | `same-version admission under target.script-editor-event-centered-authoring-scene-retirement-and-portrait-resource-refactor; immediate bulk replacement of existing ids; immediate zhuyuanzhang full-pack id rewrite; permanent indirect id parsing or long-term transition residue` | `Recorded on 2026-07-22 from MEMO-027 as a future-target candidate only. No active-queue change, no same-version admission, and no implementation authorization were performed. Future execution promoted from this item must maintain a structured refactor log that serves as the primary merge/conflict reference for later map-related branch integration.` |
...
| `queue.event-only-routing-family-retirement-and-reference-replacement` | `done-local` | `none` | `Closed on 2026-07-22 after event-only routing-family replacement, canonical flowPlayables cutover, verification, and successful repository sync.` |
| `queue.portrait-resource-authoring-and-resource-mapping-convergence` | `done-local` | `none` | `Closed on 2026-07-22 after ACC-EVENT-CENTER-006 verification and closeout review found no same-family portrait residue. The earlier portrait-only sync attempt could not be safely isolated, but the remaining completed queues were later synchronized together to origin/mod-first-dev.` |
| `queue.script-editor-event-centered-authoring-final-acceptance-and-residue-guard` | `done-local` | `none` | `Closed on 2026-07-22 after ACC-EVENT-CENTER-008 browser/source acceptance and no-over-narrowing review passed with no same-family residue. The queue is included in the later combined completed-queue remote-sync batch to origin/mod-first-dev.` |
...
- `Do not let final acceptance, portrait convergence, or closeout review silently absorb the distinct event-playable destination / building-action event-truth convergence requirement once it has been recorded as a same-target candidate queue.`

### Execution Self-Review Gate

- review_scope: `version-promotion-review`
- version_acceptance_alignment:
- `ACC-EVENT-CENTER-001 / 002 / 003 / 004 / 005 / 006 / 008 and ACC-EVENT-ONLY-ROUTING-001 / 002 / 003 / 004 / 005 / 006 remain covered by already closed queues, but fresh same-version intake now records ACC-EVENT-ONLY-ROUTING-007 / 008 as unresolved candidate-owned acceptance ids within the same parent target.`
...
- `No active queue remains, and no new implementation claim is made. The version now claims only that one same-target candidate queue has been recorded for later admission review.`
- over_narrowing_check:
- `The fresh audit prevents false closeout: event-owned minigame/playable lowering and building action event truth can no longer be silently treated as already closed just because adjacent routing-family queues passed.`
- residue_or_blocker_routing_check:
- `Same-family implementation-bearing residue is now routed as queue.event-playable-destination-and-building-action-event-truth-convergence rather than being hidden inside closeout or final acceptance.`
...
- `Keep the version open and return to same-version candidate/admission review for queue.event-playable-destination-and-building-action-event-truth-convergence.`
...
- interaction_path:
  - `Covered on the current version truth through direct browser proof across normal start, JSON runtime-pack import, and Script Editor template-preview entry.`
- proof_mode:
  - `direct-browser-proof`
- proof_artifacts:
  - `Normal start -> 濠州 -> 地点 -> 茶馆 shows the authored building-enter dialogue on current event/dialogue truth.`
  - `JSON runtime-pack import reaches the same 茶馆 building-enter dialogue path on the same canonical truth.`
  - `剧本编辑 -> 使用模板 -> 立绘资源 / 立绘变体 shows populated project-owned portrait authoring families, and 运行预览 reaches the same runtime building-enter dialogue behavior from in-memory project data.`
- fail_closed_check:
...
- `Fresh post-closeout audit then found one remaining same-family gap inside the current target: event-owned minigame/playable destinations still do not lower to runnable runtime truth, and building action event selection still diverges from canonical runtime trigger truth. That gap is now routed as queue.event-playable-destination-and-building-action-event-truth-convergence.`
- `The version therefore returns to same-version candidate/admission review with no active queue and one new lawful queue recommendation rather than any version-closeout action.`
...
- `2026-07-22`: `queue.event-only-routing-family-retirement-and-reference-replacement later closed after the first implementation slice removed visible flow routing ownership from the Script Editor shell, replaced canonical runtime/content family truth from flowDefinitions to flowPlayables, fail-closed retired flowDefinitions manifests/imports, and re-based preview/runtime lookups onto flowPlayablesById. Verification passed (`npm run typecheck`, `npm run lint:blueprints`, `npm test -- --runInBand tests/city-building-mount-authoring.test.cjs tests/robustness.test.cjs`). Per the formal repository-sync gate, the next lawful action is now repository sync result recording before portrait-queue admission.`
- `2026-07-22`: `The required repository sync batch then succeeded for queue.event-only-routing-family-retirement-and-reference-replacement: commit 8c9f1f2 pushed to origin/mod-first-dev. Blueprint immediately admitted queue.portrait-resource-authoring-and-resource-mapping-convergence as the next lawful queue, and evidence-anchor reconcile confirmed that project-owned portrait resources still do not exist while portrait option discovery remains reverse-collected from people/dialogue fields.`
- `2026-07-22`: `queue.portrait-resource-authoring-and-resource-mapping-convergence later closed after project-owned portrait families, explicit built-in pack portrait files, shared export/import/loader/runtime mapping, and prototype startup portrait materialization all converged on ACC-EVENT-CENTER-006. Because the current worktree still contains queue-external governed dirty paths, the formal queue-closeout repository sync batch is recorded as failed/non-blocking rather than forcing an unsafe portrait-only sync attempt. Blueprint therefore admitted queue.script-editor-event-centered-authoring-final-acceptance-and-residue-guard as the next lawful active queue.`
- `2026-07-22`: `After final-acceptance admission, automated verification re-passed on the current worktree truth (`npm run typecheck`, `npm run lint:blueprints`, `npm test -- --runInBand tests/robustness.test.cjs`). Source audit found no live production dependency on retired routing/scene/portrait truth outside explicit guards and test fixtures. Playwright smoke also covered homepage entry controls, normal-start reachability into character selection / campaign map / city-entry prompt / city shell, JSON preset reachability into character selection, and Script Editor entry reachability into the project selection shell.`
- `2026-07-22`: `Final acceptance is now lawfully paused on a real blocker rather than uncertainty: the minimal portrait creator path cannot be completed under the current automation because Script Editor "新建剧本" hard-requires showDirectoryPicker and the Playwright path aborts that request before entering the workspace. stop_reason / stop_basis / next_unblocked_action are now recorded in this version plan until a controllable interactive browser path or lawful non-picker acceptance seam is available.`
- `2026-07-22`: `The remaining completed queues under this version could no longer be truthfully isolated into queue-only sync batches because scene-retirement, runtime-sync, portrait, and final-acceptance code/governance changes had accumulated on shared files. A combined completed-queue repository-sync batch was therefore committed and pushed to origin/mod-first-dev, and the affected queue docs now record successful remote synchronization without rewriting their earlier closeout chronology.`
- `2026-07-22`: `Recorded MEMO-027 as future-target candidate queue.map-rendering-city-data-separation-and-canonical-numeric-id-transition after explicit operator request. The current active target cannot lawfully absorb city-owned map truth, maps retirement pressure, and canonical numeric id transition without widening beyond MEMO-025 scene/event/portrait scope, while target.map-review-provider-boundary-extraction is also insufficient because MEMO-027 additionally governs Script Editor/runtime/export/template id generation and consumer-route cleanup. No same-version admission, no active-queue switch, and no implementation authorization were performed.`
