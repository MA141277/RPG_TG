...
- `2026-08-03`: `AI collaboration governance was added as an execution-time guard for future ACC-FORMAT-006-adjacent work: AGENTS.md now points AI collaborators to docs/ai-collaboration-governance.md before scenario pack, Script Editor, event routing, building, navigation, playable/minigame, scenario-specific UI/code, or asset/resource work; the rule defaults non-framework/non-editor/non-shared-mechanism requests to scenario-pack JSON/data authoring and covers runtime/template mirroring, unified navigate routing, playable lifecycle ownership, portable resources, import/export/preview symmetry, fail-closed behavior, and verification. Focused guard verification passed with node --test tests/ai-collaboration-governance.test.cjs.`
- `2026-08-03`: `The AI collaboration governance guard was expanded with the remaining hard constraints requested by the operator: schema/contract versioning, stable ids, deletion/migration audits, runtime preview acceptance, runtime pack -> Script Editor import -> export -> runtime preview round-trip proof, reference integrity, ownership boundaries, no silent duplication, acceptance evidence, localization/text ownership, deterministic JSON ordering, explicit compatibility policy, asset size/format limits, external resource safety, and no hidden base-pack dependencies. Focused guard verification passed with node --test tests/ai-collaboration-governance.test.cjs.`
