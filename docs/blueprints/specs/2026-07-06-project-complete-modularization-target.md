...
| `queue.layout-editor-retirement-and-reference-removal` | `conditional` | `layout-editor retirement family` | `Admit only if fresh evidence proves the layout editor is still a live feature surface and its removal requires one bounded same-target queue rather than dead cleanup only.` |
