...
- Current Focus: `The intent-gate child remains completed-but-open. The latest local bugfix hardens one real gameplay seam: exact leave-house selections such as “离开酒馆” now short-circuit to a local validated route instead of depending on a remote intent-gate round trip. The unrelated full project test-script baseline still remains split across the same four external buckets.`
- Next Step: `Do not push or structurally close this child yet. First decide whether to open a separate cleanup batch for the unrelated full-suite baseline or explicitly accept leaving that baseline red.`
- Verification: `Fresh cached-Node verification for the latest leave-house hardening batch: tsc -p tsconfig.test.json PASS; .test-dist/package.json CommonJS marker write PASS; tests/npc-ai-house-intent-gate.test.cjs + tests/npc-ai-dialogue-external-provider.test.cjs + tests/npc-ai-dialogue-runtime.test.cjs PASS (50 tests, 50 pass); tsc --noEmit -p tsconfig.json PASS. The earlier full project test-script equivalent remains red in this dirty checkout with 18 unrelated/pre-existing failures outside this child’s write scope.`
...
- 2026-08-27
  ...
  - Verification: `Requested npm commands were skipped/replaced because npm is unavailable on this PowerShell PATH; cached Node equivalents were used instead. C:\Users\29636\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe tools\lint-superpowers-plans.mjs` PASS (103 files); `C:\Users\29636\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe .\node_modules\typescript\bin\tsc -p tsconfig.test.json` PASS; `Set-Content -LiteralPath .test-dist\package.json -Value '{"type":"commonjs"}' -NoNewline` PASS; `C:\Users\29636\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe --test --test-isolation=none tests/npc-ai-house-intent-gate.test.cjs tests/npc-ai-dialogue-external-provider.test.cjs tests/npc-ai-dialogue-runtime.test.cjs` PASS (47 tests, 47 pass); `C:\Users\29636\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe .\node_modules\typescript\bin\tsc --noEmit -p tsconfig.json` PASS; `C:\Users\29636\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe .\node_modules\vite\bin\vite.js build` PASS with existing asset/chunk warnings.
  - Next: `Review the final local diff, then push and add structured child closeout only after remote push succeeds.`

 
- 2026-08-27
  - Summary: `The broad whole-branch review found 2 Important remaining issues before closeout: non-OpenAI external provider modes can still bypass the hidden house intent gate, and clarify choice-loop validation still allows a response with no visible NPC follow-up line. A final fix wave is now running against those provider-contract findings, so the child is temporarily back in running state.`
  - Verification: `Controller reran the child verification batch fresh with cached Node equivalents because npm is unavailable on this PowerShell PATH: plan lint PASS (103 files), test compile PASS, focused helper/provider/runtime suites PASS (47/47), typecheck PASS, and Vite build PASS with existing asset/chunk warnings and exit 0. Final whole-branch review reported 2 Important findings and 1 deferred Minor coverage observation.`
  - Next: `Finish the final fix wave, run one scoped re-review, then restore completed-but-open status if the whole branch is clean.`

- 2026-08-27
  - Summary: `Final fix wave closed the two real provider-contract gaps: non-OpenAI external modes now fail closed for house follow-up turns before visible generation, and clarify visible replies now reject choice-only output that omits the NPC follow-up line. Scoped re-review still objected to raw marker ordering when dialogue follows `[CHOICE]`, but that finding is parked by ruling because runtime stores options separately from display pages and only exposes the options after all narration/dialogue pages have been advanced, so the live UI still shows the NPC follow-up before visible choices.`
  - Verification: `Focused post-fix verification reran fresh with cached Node equivalents because npm is unavailable on this PowerShell PATH: plan lint PASS (103 files); test compile PASS; focused helper/provider/runtime suites PASS (49/49); typecheck PASS; Vite build PASS with existing warnings and exit 0. Full project test-script equivalent (`build:test` + node --test on robustness/hardcoded-scenario-pack-boundary/character-runtime-authoring-compat/campaign-runtime-hex-coordinate-system) still fails in the current dirty checkout with 18 unrelated/pre-existing failures outside this child’s write scope.`
  - Next: `Treat this child as completed-but-open, then decide whether to fix or accept the unrelated failing full-test baseline before push/structured closeout.`

- 2026-08-27
  - Summary: `Full-suite baseline triage confirmed the remaining red tests are not one new intent-gate regression. The 18 failures currently split into four buckets: 12 robustness failures share the same createInitialState partial-input crash on missing valuables, 1 hardcoded boundary test catches new inline temple prose, 1 robustness CSS regex is drifting against the current multi-line gradient formatting, and 4 market/inventory assertions disagree with the current dirty-checkout behavior.`
  - Verification: `Reproduced the project test-script equivalent with cached Node commands because both npm and node are unavailable on this PowerShell PATH: cached-node tsc -p tsconfig.test.json PASS; .test-dist/package.json CommonJS marker PASS; cached-node --test tests/robustness.test.cjs tests/hardcoded-scenario-pack-boundary.test.cjs tests/character-runtime-authoring-compat.test.cjs tests/campaign-runtime-hex-coordinate-system.test.cjs FAIL (18). git show HEAD:src/application/state/create-initial-state.ts confirms the valuables normalization path behind the 12 crashers is unchanged in the relevant pre-child code, so that cluster is baseline drift rather than this child’s final fix wave.`
  - Next: `Keep the child completed-but-open and decide whether to spin a separate cleanup batch for the unrelated baseline or explicitly accept leaving full-suite red.`

- 2026-08-27
  - Summary: `Root-cause investigation for the reported “离开酒馆” failure showed the real proxy/model can already return the correct gate marker for that utterance, so the fragile seam was not leave-intent understanding itself but the fact that an exact leave-house selection still depended on a remote gate round trip. The provider now short-circuits exact leave-house selections such as the active leaveAction label into a local validated route with the existing fallback lead-in line instead of calling the remote intent gate first.`
  - Verification: `TDD RED/GREEN completed with cached Node commands because npm and node are unavailable on this PowerShell PATH. RED: tsc -p tsconfig.test.json PASS; .test-dist/package.json CommonJS marker PASS; node --test --test-isolation=none tests/npc-ai-dialogue-external-provider.test.cjs FAIL on the new leave-house short-circuit regression because fetchCalls was 2 instead of 0. GREEN: the same compile/package-marker steps PASS; node --test --test-isolation=none tests/npc-ai-dialogue-external-provider.test.cjs PASS (27/27); node --test --test-isolation=none tests/npc-ai-house-intent-gate.test.cjs tests/npc-ai-dialogue-external-provider.test.cjs tests/npc-ai-dialogue-runtime.test.cjs PASS (50/50); tsc --noEmit -p tsconfig.json PASS.`
  - Next: `Keep the child completed-but-open and return the next decision seam to the unrelated full-suite baseline, not this leave-house regression.`
