...
- 2026-08-05
  - Summary: `继续把 temple review advice→assign-duty 这段 follow-up 从双 owner 收成单 seam：新增 `createTempleReviewAdviceFollowupProjection(...)`，把 review work choices、advice acknowledge 文案、special-task 描述与 assign-duty 台词统一封装成一处宿主 projection helper；hosted `advice` stage handoff 和 `handleLegacyTempleReviewFallback(...)` 现在都只消费同一个 helper，不再各自直接拼 assign-duty 的 meetingStage/dialogueLines/overlay。`
  - Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "temple review advice-to-assign-duty follow-up projection is owned by one helper across hosted and fallback paths" tests/robustness.test.cjs` first failed because `createTempleReviewAdviceFollowupProjection(...)` did not exist, then passed after the extraction; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 tests/temple-meeting-runtime-integration.test.cjs` passed 6/6; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 --test-name-pattern="temple review" tests/robustness.test.cjs` passed 8/8.`
  - Next: `继续审计 temple 剩余 no-meeting fallback 的 review 分支，优先删除或抽离仍会直接构造本地 projection 的尾部段落，直到 temple review 只保留明确的宿主 settlement seam 与 no-meeting 入口兜底。`
- 2026-08-05
  - Summary: `继续收窄 temple review 的双 caller seam：新增 `createTempleReviewAssignDutyActionContainer(...)`，把 assign-duty 阶段的标题、review work choice 映射、disabled/tone 透传和按钮渲染元数据统一封装成单一 helper；`projectTempleHostedReviewStage(...)` 与 temple `selectViewModel(...)` 不再各自直接 `reviewWorkChoices.map(...)`，而是只通过这一个 helper 分别注入 hosted action id 和 legacy fallback action id。`
  - Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "temple review assign-duty action container is owned by one helper across hosted and fallback shells" tests/robustness.test.cjs` first failed because `createTempleReviewAssignDutyActionContainer(...)` did not exist, then passed after the extraction; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run build:test` passed; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 tests/temple-meeting-runtime-integration.test.cjs` passed 6/6; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 --test-name-pattern="temple review" tests/robustness.test.cjs` passed 9/9.`
  - Next: `继续回到 temple legacy fallback 的尾部转场，优先整理 intro→assignment-table 与 assigned/finished closeout 这些仍未明确收敛到 helper 或宿主 settlement seam 的残段。`
- 2026-08-05
  - Summary: `继续把 temple legacy fallback 前段转成 projection seam：新增 `createTempleReviewAssignmentTableProjection(...)`，把 `intro -> assignment-table` 的 reviewProgress lead、assignment rows 和 overlay 组装收口到单一 helper；`handleLegacyTempleReviewFallback(...)` 的 `intro` 分支不再直接写 assignment-table 的本地 stage 数据，而是改为消费 projection 并统一走 `applyTempleHostedReviewProjectionToSessionState(...)`。`
  - Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "temple review assignment-table fallback projection is owned by one helper" tests/robustness.test.cjs` first failed because `createTempleReviewAssignmentTableProjection(...)` did not exist, then passed after the extraction; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run build:test` passed; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 tests/temple-meeting-runtime-integration.test.cjs` passed 6/6; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 --test-name-pattern="temple review" tests/robustness.test.cjs` passed 10/10.`
  - Next: `继续处理 temple 剩余 assigned / finished closeout 壳，明确哪些还能收成 helper，哪些要作为宿主 settlement seam 固化。`
- 2026-08-05
  - Summary: `继续把 temple review 的宿主 settlement seam 收口到单一 helper：新增 `createTempleAssignedSettlementSessionState(...)`，把 `meetingStage: "assigned"` 结果壳的 mode、selectedTaskId、selectedWorkPlan、dialogueLines 与 success overlay 统一封装；`submitReviewWorkPlan(...)` 和 `assignTempleTask(...)` 现在都不再各自拼 assigned session，而是只负责生成具体文案和任务/工作计划数据，然后交给同一个宿主 helper 生成结果壳。`
  - Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "temple assigned settlement shell is owned by one helper across review settlement paths" tests/robustness.test.cjs` first failed because `createTempleAssignedSettlementSessionState(...)` did not exist, then passed after the extraction; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run build:test` passed; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 tests/temple-meeting-runtime-integration.test.cjs` passed 6/6; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 --test-name-pattern="temple review" tests/robustness.test.cjs` passed 10/10.`
  - Next: `审计当前 plan 是否还剩值得继续切的 shared/host seam；若只剩单一路径 closeout，则结束 generic meeting 收敛并切到下一条 runtime/event 迁移计划。`
- 2026-08-05
  - Summary: `继续把 legacy fallback 的 praise-stage 跟进转场收成 projection seam：新增 `createTempleReviewPraiseStageFollowupProjection(...)`，把 `praise -> situation`、`situation -> policy`、`policy -> advice` 以及 `close-review-policy-panel -> advice` 统一封装为一组投影结果；`handleLegacyTempleReviewFallback(...)` 不再自己散着写 `situation/policy/advice` 的本地 session patch，而是统一消费该 helper 并通过 `applyTempleHostedReviewProjectionToSessionState(...)` 回写。`
  - Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "temple praise-stage fallback follow-up projection is owned by one helper" tests/robustness.test.cjs` first failed because `createTempleReviewPraiseStageFollowupProjection(...)` did not exist, then passed after the extraction; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run build:test` passed; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 tests/temple-meeting-runtime-integration.test.cjs` passed 6/6; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-concurrency=1 --test-name-pattern="temple review" tests/robustness.test.cjs` passed 10/10.`
  - Next: `确认当前 generic meeting plan 是否只剩宿主单路径 closeout；如果是，就收束这条 plan 并切换到下一个 runtime/event 迁移计划。`
