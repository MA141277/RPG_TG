...
- Current Focus: `The event-runtime payload convergence checkpoints remain merged and pushed through a526dffc. The latest adjacent runtime-only child now locks story progression timing against the canonical ISO occurredAt seam: story-runtime progression updatedAt is emitted as YYYY-MM-DDT00:00:00.000Z, and the new story-runtime progression timing regression proves settlement-driven tier evaluation still reads post-settlement metrics before emitting creator-authored target-tier settlement work.`
- Next Step: `Commit and push the story-runtime progression timestamp convergence checkpoint, then continue the next runtime-only event-system migration slice.`
...
- 2026-07-31
  - Summary: `Restored the script-editor runtime preview startup seam that surfaced during browser smoke. MainUiFlow now receives onStartLoadedScenarioPack/onExitRuntimePreview from main.ts, startup-session-coordinator accepts a loaded-scenario-pack request for already-loaded preview packs, and runtime preview startup sanitizes deferred entry-event timing before reusing the shared startup-session flow.`
  - Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run build:test` passed (exit 0); `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm exec node --test tests/script-editor-entry-availability.test.cjs tests/startup-session-coordinator.test.cjs tests/scenario-preview-sanitizer.test.cjs` passed 7/7; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run typecheck` passed (exit 0); `git diff --name-only -- src/main.ts src/ui src/components src/application/map src/application/backpack src/domain/backpack src/domain/map src/styles` reported only approved `src/main.ts`; `git diff --check` returned empty output.`
  - Next: `Commit and push this runtime-preview shell wiring checkpoint, then continue the next runtime-only event-system migration slice.`

- 2026-07-31
  - Summary: `Added a story-runtime progression timing regression and converged story progression timestamps onto the canonical ISO occurredAt seam. The new test proves settlement-driven progression still evaluates post-settlement metrics, while story-runtime now writes trackState.updatedAt as YYYY-MM-DDT00:00:00.000Z instead of the older date-only string.`
  - Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run build:test` passed (exit 0); `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm exec node --test tests/story-runtime-progression-timing.test.cjs tests/progression-runtime.test.cjs` passed 4/4; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run typecheck` passed (exit 0); `git diff --name-only -- src/main.ts src/ui src/components src/application/map src/application/backpack src/domain/backpack src/domain/map src/styles` returned empty output; `git diff --check` returned empty output.`
  - Next: `Commit and push this progression timestamp convergence checkpoint, then continue the next runtime-only event-system migration slice.`

- 2026-07-30
  - Summary: `Opened and completed the next runtime-only child for event-runtime candidate task-input payload consumption. event-runtime now builds EventRuntimeCandidate.taskInputs through shared readRuntimeEventTaskInputs(...) on projected runtime-event payloads, so this seam no longer rereads authored eventDefinition.taskInputs.`
...
- 2026-07-30
  - Summary: `Opened and completed the next runtime-only child for runtime event binding action payload consumption. event-binding-runtime now applies binding-owned runtime actions through shared readRuntimeEventActions(...) on projected runtime-event payloads, so this seam no longer rereads authored eventDefinition.actions.`
...
- 2026-07-30
  - Summary: `Opened and completed the next runtime-only child for runtime event dialogue-id payload consumption. Story-runtime and event-binding-runtime now classify action-only/state-only events through shared readRuntimeEventDialogueId(...) on projected runtime-event payloads, so this seam no longer rereads authored eventDefinition.dialogueId.`
