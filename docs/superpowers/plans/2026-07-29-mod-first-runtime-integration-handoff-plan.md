...
- Current Focus: `Scene-runtime task input convergence is fully implemented and pushed as 9acb1ae on codex/migration-hot-tasks. The next immediate action is to merge this checkpoint back to the aligned baseline; the next adjacent runtime-only candidate after that is runtime-dispatch split task input compatibility narrowing.`
- Next Step: `Merge 9acb1ae back to codex/sync-naqishuo-721ui-to-mmz-followup and origin/codex/sync-naqishuo-721ui-to-mmz, then decide whether to open the dispatch-compatibility child immediately.`
- Verification: `The task-input child is fully merged back at 91780be and recorded in docs/superpowers/plans/2026-07-30-runtime-task-input-contract-convergence-plan.md; for the scene-runtime child, pnpm run build:test, the focused robustness subset, typecheck, boundary diff, and git diff --check are green, while lint:plans still fails only on the unrelated docs/superpowers/plans/2026-07-23-haozhou-coin-ingot-flight.md title-heading issue and the broader targeted robustness invocation still carries the known unrelated runtime-router route-result alias assertion.`
