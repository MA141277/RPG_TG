...
- Current Focus: `The dedicated city-enter follow-up child is now implemented and verified on codex/migration-hot-tasks: navigation-time-follow-up uses shared story-runtime instead of scene-runtime, outcome follow-up forwarding now carries city/building status patches, and the only guarded shell-path touch remains a narrow navigationTimeFollowUp context passthrough in src/main.ts.`
- Next Step: `Review the completed child at docs/superpowers/plans/2026-07-29-navigation-time-follow-up-story-runtime-city-enter-plan.md, then decide whether to commit/push that completed-but-open slice now or keep it open for another adjacent runtime-only batch.`
