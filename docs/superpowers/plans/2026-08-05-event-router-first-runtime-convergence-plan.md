...
- Current Focus: `Payload-first third slice landed: runtime-pack export/import now also distinguishes destination-derived city-menu actions from explicit authored \`openCityMenuPanel\` payload actions, so menu payload actions no longer collapse into \`destination.family = "menu"\` during Script Editor round-trip.`
- Next Step: `Continue payload-first convergence after playable/menu destination disambiguation: likely mixed destination/action validation for menu+playable combinations, richer event action normalization in authoring, or the next direct caller family that still bypasses router/runtime-dispatch.`
- Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run build:test` passed; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "runtime-pack round trip preserves menu destinations for retained template events|runtime-pack round trip keeps explicit openCityMenuPanel actions on the payload seam" tests/script-editor-runtime-preview-compat.test.cjs` passed 2/2; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "runtime-pack import keeps playable action rehydration off the destination seam|runtime-pack import keeps menu action rehydration off the destination seam" tests/robustness.test.cjs` passed 2/2.`
