...
- Current Focus: `Payload-first eighth slice landed: explicit \`openCityMenuPanel\` payload actions now validate and normalize panel ids on export, so unsupported values fail closed instead of leaking malformed menu routes into runtime packs.`
- Next Step: `Continue payload-first convergence after panel-id validation: likely broader action normalization in authoring, explicit launchPlayable/integration validation, or the next direct caller family that still bypasses router/runtime-dispatch.`
- Verification: `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" pnpm run build:test` passed; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "runtime-pack export rejects openCityMenuPanel actions with unsupported panel ids" tests/script-editor-runtime-preview-compat.test.cjs` passed 1/1; `PATH="/Users/ms/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin:$PATH" node --test --test-name-pattern "runtime-pack export fails closed on mixed destination and same-family payload actions" tests/robustness.test.cjs` passed 1/1.`
