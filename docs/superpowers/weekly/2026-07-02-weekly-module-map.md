...
- Can the bounded `city-enter` story-trigger follow-up now be converged cleanly as part of the next event/scene child instead of another navigation child?
- Should the retained council-priority follow-up stay shell-side, or does the post-Child-15 baseline justify a later state-sync or event/time-specific extraction?
