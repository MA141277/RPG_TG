...
2. Which module was harder to change than expected?
   - The Child 4 interactive seam request factories were harder than expected because `exactOptionalPropertyTypes` rejected explicit `payload: undefined`, so the bridge helpers had to use conditional spreads rather than blindly serializing empty payload fields.
3. Which module likely needs a new seam or contract?
   - `src/core/runtime`, because the new Child 4 bridge seams exist but the remaining architecture question is whether interactive request ids should now join the shared router/dispatch contract instead of staying on dedicated helper entry points.
