...
| Item | Current Truth |
| --- | --- |
| next executable child | `docs/superpowers/plans/2026-07-01-sub-runtime-ownerization-implementation-plan.md` |
| queued review child | `none` |
| locked implementation child | `none` |
| may Child 10 start now | `n/a (completed)` |
| may Child 11 start now | `yes` |
| Child 11 unlock owner | `already satisfied by Child 10 baseline + Child 11 spec/plan + weekly queue sync` |

## 3. Consistency Matrix

| Concern | Canonical Statement | Primary Source | Supporting Source(s) | Quick Audit Question |
| --- | --- | --- | --- | --- |
| active next child | Child 10 is completed, Child 11 spec/plan now exist against the finalized baseline, and Child 11 is the next executable ownerization child. | `weekly-orchestration-plan.md` | `runtime-ownerization-baseline.md`, `weekly-review-index.md`, `weekly-implementation-visibility-plan.md` | Do all current-state docs agree that Child 11 is now executable but not yet started? |
...
| Child 11 status | Child 11 is `not-started` and formally unlocked because its spec/plan and weekly unlock sync now exist. | `weekly-orchestration-plan.md` | `weekly-review-index.md`, `weekly-next-split-review.md`, `weekly-implementation-visibility-plan.md` | Does any doc still incorrectly describe Child 11 as locked after the unlock batch? |
...
| visibility sync rule | Weekly artifacts must preserve one queue truth: `Child 10(completed) -> Child 11(unlocked, not-started, ready to begin from its own plan)`. | `weekly-implementation-visibility-plan.md` | `weekly-review-index.md`, `weekly-next-split-review.md`, `weekly-architecture-report.md`, `weekly-module-map.md` | Do all five weekly artifacts describe the same post-unlock sequence? |
...
- a doc still says Child 11 is `locked` after the unlock batch
...
- [ ] weekly orchestration plan records `Child 10` as completed and `Child 11` as unlocked and not-started
...
- [ ] Child 11 spec and plan are present and referenced
- [ ] Child 11 is described as unlocked and not-started
