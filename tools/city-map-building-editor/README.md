...
编辑器现在使用固定 `20 x 20` 等距菱形棋盘作为主放置模型，不再依赖自动识别城墙内部范围。
...
`map.optionalBuildableMask` 只作为“城墙参考遮罩”显示，默认关闭，不参与建筑能否放置的判断。旧 JSON 中的 `map.buildablePolygon` 导入时会被兼容读取为 `optionalBuildableMask`。
