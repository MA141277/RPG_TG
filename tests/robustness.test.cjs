...
test("yuanmo hex map editor exposes sampling step and layered editing controls", () => {
  ...
  assert.match(source, /sourceImage:\s*session\.sourceImageState\s*===\s*"ready"/);
  assert.match(source, /viewportMode:\s*session\.activeStep\s*===\s*"crop"\s*\?\s*"source-map"\s*:\s*"manual-hex"/);
...
test("campaign terrain WebGL shader uses separate shared animated water texture files", () => {
  ...
  assert.match(rendererSource, /getCampaignTerrainChunkKeysForCells/);
...
test("map3 runtime export keeps gameplay hex size and one-to-one cells", () => {
  ...
  assert.equal(appliedWaterLandOverrides.length, 38);
...
test("campaign fog exploration stays active without the removed shader renderer", () => {
  ...
  assert.doesNotMatch(
    cloudRevealMaskSource,
    /projectCampaignTerrainUvToClientPointAtCloudRevealHeight|getCampaignTerrainProjectionSignature|projectCampaignTerrainUvToClientPoint\(\s*input\.projectionRoot,/,
...
test("temple review advice follow-up projection is owned by one helper across hosted and fallback paths", () => {
  const moduleSource = fs.readFileSync(
    path.join(
      process.cwd(),
      "src",
  ...
  const legacyBlock =
    functionBlocksByName.get("handleLegacyTempleReviewFallback") ?? "";
  const hostedBlockMatch = moduleSource.match(
    /const hostedAdviceStageHandoff =[\s\S]*?const hostedMeetingResult =/
  );
  const hostedBlock = hostedBlockMatch?.[0] ?? "";
  ...
  assert.match(helperBlock, /getDefaultReviewSpecialTaskHookResult\(/);
  ...
  assert.doesNotMatch(hostedBlock, /getDefaultReviewSpecialTaskHookResult\(/);
...
test("temple review policy close remains fallback-only while hosted path uses one advance seam", () => {
  const moduleSource = fs.readFileSync(
    path.join(
      process.cwd(),
      "src",
  ...
  assert.match(hostedAdvanceHelperBlock, /close-review-policy-panel/);
  assert.match(hostedRequestBlock, /isTempleHostedMeetingAdvanceAction\(actionId\)/);
  assert.match(legacyBlock, /if \(actionId === "close-review-policy-panel"\)/);
...
test("runtime event task input payload consumption keeps story-runtime on the routed payload seam", () => {
  const projectionSource = fs.readFileSync(
    path.join(process.cwd(), "src/core/runtime/event-entity-projection.ts"),
    "utf8"
  );
  ...
  const routedHandlersBlock =
    storyRuntimeSource.match(
      /handlers:\s*\{[\s\S]*?dialogue:\s*\(\{ state, event \}\)\s*=>[\s\S]*?settlement:\s*\(\{ state, event \}\)\s*=>[\s\S]*?\n\s*\},/
    )?.[0] ?? "";

  assert.match(projectionSource, /export function readRuntimeEventTaskInputs\(/);
  assert.match(projectionSource, /payload\.taskInputs/);
  assert.match(storyRuntimeSource, /readRuntimeEventTaskInputs/);
