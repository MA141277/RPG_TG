...
test(
  "script editor settlement layout keeps compact cards and top-right remove controls",
  { timeout: 120000 },
  async (t) => {
    ...
    assert.equal(await page.locator(".c-script-editor-settlement-panel").count(), 1);
    assert.equal(await page.locator(".c-script-editor-settlement-content-list").count(), 1);
    assert.equal(await page.locator(".c-script-editor-settlement-content-card").count() > 0, true);
    assert.equal(
      await page.locator(".c-script-editor-settlement-content-card__remove").count() > 0,
      true
    );

    const cardCountBefore = await page.locator(".c-script-editor-settlement-content-card").count();
    await page.locator('[data-script-editor-action="add-settlement-content"]').click();
    await page.waitForTimeout(150);
    const cardCountAfterAdd = await page.locator(".c-script-editor-settlement-content-card").count();
    assert.equal(cardCountAfterAdd, cardCountBefore + 1);
    await page.locator(".c-script-editor-settlement-content-card__remove").last().click();
    await page.waitForTimeout(150);
    const cardCountAfterRemove = await page.locator(".c-script-editor-settlement-content-card").count();
    assert.equal(cardCountAfterRemove, cardCountBefore);

    assert.deepEqual(pageErrors, [], "Settlement layout smoke should not raise uncaught page errors.");
  }
);
