...
test("battle UI editor ignores stale persisted defaults from earlier layouts", () => {
  const main = read("src/main.ts");

  assert.match(
    main,
    /const STALE_BATTLE_UI_EDITOR_DEFAULTS:\s*Partial<\s*Record<BattleUiEditorVariableName,\s*string>\s*>\s*=\s*\{/
  );
  assert.match(main, /"--battle-right-panel-left":\s*"72\.6074%"/);
  assert.match(main, /"--battle-right-panel-top":\s*"12\.5%"/);
  assert.match(
    main,
...
test("battle demo right corner button labels stay inside their image frames", () => {
  ...
  assert.match(main, /"--battle-controls-button-height":\s*"6\.1%"/);
  assert.match(main, /"--battle-start-left":\s*"78\.35%"/);
  assert.match(main, /"--battle-start-top":\s*"92\.52%"/);
  assert.match(main, /"--battle-start-width":\s*"9\.45%"/);
  assert.match(main, /"--battle-end-turn-left":\s*"89\.55%"/);
  assert.match(main, /"--battle-end-turn-top":\s*"92\.46%"/);
  assert.match(main, /"--battle-end-turn-width":\s*"9\.45%"/);
});
