...
test("temple-copy-scripture is registered as a canonical playable shell", () => {
  ...
  const contractSource = fs.readFileSync(
    path.join(process.cwd(), "src/minigames/temple-copy-scripture/contract.ts"),
    "utf8"
  );

  assert.match(registrySource, /templeCopyScripturePlayableShell/);
  assert.match(definitionSource, /templeCopyScripturePlayableShell\.manifest\.commandPrefix/);
  ...
  () => {
    const launched = runPlayableRuntime({
      ...
      request: createLaunchPlayableRequest("temple-copy-scripture", {
        integrationId: "playable.temple-copy-scripture.house.temple",
        ownerContext: {
          ownerKind: "house",
