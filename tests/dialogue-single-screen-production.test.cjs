...
test("runtime pack import migrates legacy narration-only dialogues onto a previewable single-screen fallback", () => {
  const project = importScenarioPackToScriptEditorProject({
    schemaVersion: 1,
    id: "pack.dialogue.legacy-narration-import",
    title: "Legacy Narration Import",
    scenarioProfile: {
      id: "scenario.dialogue.legacy-narration-import",
      playerCharacterId: "char.hero",
      chapterId: "chapter.start",
      initialLocation: {
        mapId: "map.test",
    ...
    characters: [{ id: "char.hero", name: "Hero" }],
    cities: [{ id: "city.test", name: "Test City", houseIds: [] }],
    dialogues: [
      {
        id: "dialogue.legacy.narration",
        name: "Legacy Narration",
        nodes: [
          {
            id: "node.narration.1",
            type: "narration",
            textId: "text.narration.1",
          },
          {
            id: "node.narration.2",
            type: "narration",
            textId: "text.narration.2",
          },
        ],
      },
    ],
    textEntries: {
      "text.narration.1": "第一段旁白",
      "text.narration.2": "第二段旁白",
    },
    events: [
      {
        id: "event.narration",
        ...
        destination: {
          family: "dialogue",
          targetId: "dialogue.legacy.narration",
        },
      },
    ],
  });

  assert.equal(project.dialogues[0].mode, "linear");
  assert.equal(project.dialogues[0].textId, "text.narration.1");
  assert.equal(project.dialogues[0].speakerPersonId, "char.hero");
  assert.deepEqual(project.dialogues[0].cast, [
    { personId: "char.hero", side: "left" },
  ]);

  const diagnostics = validateScriptEditorProjectForRuntimeExport(project);
  assert.deepEqual(
    diagnostics.filter((diagnostic) =>
      diagnostic.fieldPath.startsWith("project.dialogues[0].speakerPersonId") ||
      diagnostic.fieldPath.startsWith("project.dialogues[0].cast") ||
      diagnostic.fieldPath.startsWith("project.events[0].destination.targetId")
    ),
    []
  );
});
...
test("project parsing safely migrates a legacy one-node dialogue into single-screen fields", () => {
  const project = createDefaultScriptEditorProjectDefinition();
  project.dialogues = [
    {
      id: "dialogue.legacy.linear",
      title: "旧线性对话",
      storyNodeId: "story.node.legacy",
  ...
  assert.equal(dialogue.mode, "linear");
  assert.equal(dialogue.textId, "text.dialogue.legacy");
  assert.equal(dialogue.speakerPersonId, "person.hero");
  assert.deepEqual(dialogue.cast, [{ personId: "person.hero", side: "left" }]);
  assert.equal(dialogue.nextEventId, "event.followup");
  assert.equal(Object.hasOwn(dialogue, "participantPersonIds"), false);
  assert.equal(Object.hasOwn(dialogue, "nodes"), false);
