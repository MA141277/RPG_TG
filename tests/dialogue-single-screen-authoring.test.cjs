...
test("dialogue materializer rejects retired node-based lowering on the main export path", () => {
  const result = materializeScriptEditorDialogueStoryRuntime({
    people: [{ id: "char.hero", title: "主角" }],
    events: [],
    storyNodes: [],
    textEntries: [{ id: "text.legacy", text: "旧节点对话" }],
    dialogues: [
      {
        id: "dialogue.legacy.nodes",
        title: "旧节点对话",
        nodes: [
          {
            id: "node.start",
  ...
  assert.deepEqual(result.diagnostics, [
    {
      ...
      message:
        'Dialogue "dialogue.legacy.nodes" still relies on retired node-based export fields; migrate it onto the single-screen dialogue model before runtime export.',
    },
  ]);
});
