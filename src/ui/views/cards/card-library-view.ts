import type { CardDefinition, CardInventory } from "../../../domain/card";
import type { CardLibraryFilter } from "../../../domain/global-ui";

const CARD_FILTER_LABELS: Record<CardLibraryFilter, string> = {
  all: "全部",
  "secret-technique": "秘技",
  battle: "合战",
};
...
export function renderCardLibraryView(input: {
  cardDefinitions: CardDefinition[];
  inventory: CardInventory;
  filter: CardLibraryFilter;
}): string {
  ...
  const selectedCard =
    visibleCards.find((cardDefinition) => cardDefinition.id === input.inventory.selectedCardId) ??
    visibleCards[0] ??
    null;
