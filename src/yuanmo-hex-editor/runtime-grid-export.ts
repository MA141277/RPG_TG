...
import { YUANMO_GRASS_ENVIRONMENT, YUANMO_PLAIN_TERRAIN } from "./model";
...
function resolveEditorCells(input: RuntimeGridExportInput): GeneratedHexCell[] {
  ...
  return input.editorGenerated.cells.map((cell) => {
    ...
    return {
      ...
      terrain: land ? terrainOverride?.terrain ?? cell.terrain : YUANMO_PLAIN_TERRAIN,
      environment: land
        ? environmentOverride?.environment ?? cell.environment
