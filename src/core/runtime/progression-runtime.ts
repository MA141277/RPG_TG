...
export function runProgressionRuntime(input: {
  ...
  const didChangeTier =
    (current?.currentTierId ?? null) !== (nextTier?.id ?? null);
  const enteredTierHistory =
    didChangeTier && nextTier != null
...
function selectTargetTier(input: {
  ...
  const currentTier =
    input.currentTierId == null
      ? null
      : input.track.tiers.find((tier) => tier.id === input.currentTierId) ??
        null;
  if (currentTier == null || highestSatisfiedTier == null) {
    return currentTier ?? highestSatisfiedTier;
  }
