...
export function createDefaultPlayableRuntimeRegistries(): PlayableRuntimeRegistries {
  return {
    definitions: createPlayableDefinitionRegistry(
      createBuiltinPlayableDefinitionRegistry().entries()
    ),
    integrations: createPlayableIntegrationRegistry(
      createBuiltinPlayableIntegrationRegistry().entries()
    ),
  };
}
...
export function createPlayableRuntimeRegistriesFromActivatedMod(
  ...
  const contributedPlayableIds = new Set(
    activatedMod.gameplayContributions.playables
  );
  ...
  for (const source of activatedMod.normalizedContentSources) {
    ...
    for (const definition of readPlayableDefinitions(
      contributionSource.playables
    )) {
...
function readPlayableIntegrations(
  value: unknown
): PlayableIntegrationDefinition[] {
  if (!Array.isArray(value)) {
    return [];
