...
exports.createGlobalPlayerPanelModel = createGlobalPlayerPanelModel;
exports.renderGlobalPlayerPanel = renderGlobalPlayerPanel;
function createGlobalPlayerPanelModel(playerCharacter, state, activeMission) {
    const notorietyValue = state.runtime.variables.notoriety;
    return {
        portraitLabel: playerCharacter.portraitVariants?.find((variant) => variant.id === playerCharacter.portraitVariantId)?.label ?? "通常",
...
function renderGlobalPlayerPanel(model) {
    return `
