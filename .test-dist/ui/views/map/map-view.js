...
exports.renderMapView = renderMapView;
function createMapViewModel(input) {
    return {
        ...
        cityMarkers: input.cityDefinitions
            ...
            .filter((cityMarker) => cityMarker != null),
    };
}
function renderMapView(model) {
    const cells = [];
    for (let y = 0; y < model.size; y += 1) {
        for (let x = 0; x < model.size; x += 1) {
            const cityMarker = model.cityMarkers.find((marker) => marker.x === x && marker.y === y);
            const isPlayerHere = model.playerCoordinate.x === x && model.playerCoordinate.y === y;
            cells.push(`
        <button
